...
## D-0030 · 2026-07-31 · The calibration log gets a counted vocabulary and a positional boundary

**Decision:** Two changes to what the minor-recalibrate trigger means. (a) It counts a row only when the row's miss-type **begins with a documented token**; rows written as parenthetical annotations are recorded but never counted. The token list in `templates/learner/calibration-log.md` and `MISS_TYPES` in `tools/primer_state.py` are asserted equal by a test. (b) Counting starts from a **positional marker** written by `recalibrate-mark` into the calibration log itself, not from a date comparison against `log.md`. Two tokens are added: `register-miss` and `analogy-transfer`.

**Context:** Both halves were wrong in the same live run, in opposite directions, and the errors cancelled into plausible-looking output.

Counting *dated lines* meant every row counted, including the ones the template invites you to write as commentary. The trigger fired at "5 misses ≥ 4" when two of the five were annotated `(intake floor-finding, not a miss)` and `(mastery signal, not a miss)`. The inversion is the part that matters: **a demonstration of mastery raised the apparent need to correct the model** — the strongest evidence the log carries, pushing the loop the wrong way. This is the same class as the two template-drift bugs in Session 3 and D-0029's five: code and a document disagreeing about what a row means, with the document inviting exactly the input the code mishandles. Hence the equality test rather than a comment asking future editors to keep two lists in sync.

Comparing *dates across two files* meant everything logged on the same calendar day as a recalibrate was invisible. That is not an edge case: a minor recalibrate runs at session start and the session's misses are written at its end, so **the common case dropped a whole session's evidence.** After the marker fix, the instance that had been reporting `misses=0` reported `misses=8`. A positional marker inside the file the misses live in has no same-day ambiguity and needs no clock; the cost is one more command at the end of the recalibrate, which the protocol now names as step 5.

The two new tokens exist because the protocol asked for signals it gave no way to record. `feedback-protocol.md` instructs the Primer to infer *"style confirmation — did the chosen register / correction style / narrative density fit"*, and there was no token for the answer; two register corrections arrived in consecutive turns in one lesson with nowhere to go. `analogy-transfer` is separated from `dead-analogy` because the correction differs: a dead analogy failed to connect and should be replaced, while a transfer miss connected, was productive, and mispriced something — you keep the analogy and fix the number. Both appeared as `(NEW TOKEN — not in the documented list)` in a live log before being added, which is the right order.

**Alternatives:** filter annotation rows by looking for "not a miss" in the free text (rejected — unparseable prose, and it invites a second vocabulary nobody documents); count everything and raise the threshold (rejected — leaves mastery signals pushing the trigger the wrong way, which is the actual defect); timestamps instead of a marker (rejected — the state files are human-authored and hand-editable, and demanding a clock time in every row taxes the writer to fix a boundary problem a marker solves); infer the boundary from `log.md` position (rejected — the boundary must live in the file being counted, or cross-file edits reintroduce the ambiguity).

**Tradeoff:** an undocumented token is now silently uncounted rather than loosely counted, which is a real failure mode — mitigated by the equality test and by writing new tokens into the log as `(NEW TOKEN …)` when they first appear. And `recalibrate-mark` is a step that can be forgotten; when it is, counting falls back to the date comparison, now `>=` rather than `>`, which recovers the same-day entries that motivated the change.
...
**Context (a):** D-0015 named the closed-self-assessment problem and added two anchors, but cold retrieval and time decay both anchor the *learner's recall*. Neither checks the *Primer's judgment*, and that is the weaker half. Models judge a **correct** answer at 94–99% F1, a **valid-but-unconventional** one at 0–76%, and an **incorrect** one at 4–55%, with over-validation to 71% — and giving the model the full solution changes nothing (arXiv:2605.16207). The valid-but-unconventional answer is what a senior learner produces, so primer's normal case sat in the worst measured band, unexamined. Separately, an 8B model with an adversarial critic panel beat GPT-4o on pedagogical judgment, and removing the devil's-advocate role cost more than removing fine-tuning (AAAI 2026, arXiv:2512.22496): the fix that works is architectural separation, not a better prompt.

Two details are the mechanism, not decoration. **Withholding the proposed delta** — an examiner shown "the Primer wants to raise this" is being asked to ratify, and the sycophancy evidence says it will. And **disagreement holds rather than resolves**: averaging two reads, or letting the Primer adjudicate its own case, reintroduces exactly the drift. The asymmetry is deliberate — contested evidence blocks an upgrade but not a downgrade, because an upgrade on contested evidence *is* the optimism drift.

**Context (b):** "hold correct positions under pushback; sycophancy is failure" was a non-negotiable with no test. It is now testable, and the uncomfortable part is where: the measured weak mode for Claude models is the **context-switch frame attack, worst at low learner confidence** (arXiv:2605.14604) — which is precisely what a senior-peer register invites and what a low-confidence depth marker describes. The register that makes primer good maximises its measured failure mode, so `anti-patterns.md` #1 gains a per-mode counter-move, with restoring the instructional frame named explicitly.

The scorer **never collapses to one number**, because the finding that motivates the eval is that aggregates hide the failure: two models with the same ~14% overall rate failed on opposite modes, and single domain×mode cells spiked past 30%. An unrun trap raises rather than scoring as a pass — counting it would understate the rate the eval exists to measure — and coverage gaps print by name.

**Context (c):** T7's hatch is right (misaligned Socratic pressure causes overload), but a *rising* rate is the AI-dependence signal the cognitive-offloading literature describes, and nothing counted it. The denominator is **lessons, not calendar time**: a learner who takes a month off has not become more dependent, and a rate over days would say they had. It stays an ordinary calibration miss so it feeds the existing recalibrate trigger — repeated hatch use means the register is mis-set, which is what the minor recalibrate is for.

**Alternatives:** a full five-act critic panel per lesson (rejected — validated for *batch grading* at ~10× latency; one examiner at Recap takes the part that fits and leaves the debate, which would wreck the conversational loop); a neutral second opinion instead of an adversarial one (rejected — the ablation says the adversarial framing carries more weight than model quality); a headless harness that runs the traps automatically (deferred — running a trap needs a live tutor and a judgement call, so the split is agent-runs / code-scores); a separate habits log for hatch use (rejected — it *is* a struggle-tolerance miss, and splitting it would hide it from the trigger that should act on it).

**Tradeoff:** one subagent call per lesson and a trap set to maintain, in exchange for the first real check on the Primer's own judgment and the first test of a non-negotiable. Stated limit: the examiner reads the same transcript through the same kind of model, so a misconception both share stays invisible — it narrows the gap between the Primer's opinion and the learner's demonstrated ability without closing it. The signal that closes it is the one `feedback-protocol.md` already says to prefer: the learner reporting they applied a pattern in real work. Implements Proposal 0003 Wave B (H1–H3).

## D-0026 · 2026-07-30 · Self-update is one verb that updates the engine *and* migrates the instance; the notice rides skill invocation, not a hook

**Decision:** `/primer update` (`tools/primer_update.py`) does both halves in one step — fast-forwards the core checkout and migrates the learner's private instance — and the "an update is waiting" notice is delivered by **SKILL.md's `` !`…` `` dynamic-context injection**, cached to at most one network call a day. A `VERSION` file in the core is the version marker.

**Context:** two problems, one of them already live. The learner-facing one: primer's install is a git clone plus a symlink, so updating meant knowing `git pull`, and the audience explicitly includes people who don't. The engineering one, which is worse: **a core update alone can leave a working instance broken.** Wave E added `templates/learner/source-ledger.md`, and every instance created before it lacks `learner/source-ledger.md`, so `primer_sources.py` errors on first use. Verified on the maintainer's own instance the moment `status` existed. An updater that only pulls the engine would have shipped that breakage to every user.

So migration is not a nicety bolted onto the updater — it is half of what "update" means here, and it only ever *adds*: a missing state file is copied from the template, a missing gitignore rule is appended, and an existing file is never touched, because an existing file holds the learner's data and the template is an empty scaffold.

**Why injection rather than a hook.** GSD's model is npm + a runtime hook; primer can't copy it. A `SessionStart` hook is an extra install step for a non-technical user, it edits a global settings file, and it would fire in every session whether primer is involved or not. The injection rides the moment the learner is already running primer, needs no setup, and costs a file read. It prints nothing when up to date **and nothing when it can't tell** — offline or on a fork must read as "don't know" and stay silent, never as up-to-date and never as an error in front of a learner.

**Refuse rather than guess.** The updater declines on uncommitted changes, diverged local commits, a missing git, or a core that isn't a checkout — each with the specific next step. SKILL.md tells the model explicitly not to work around a refusal by force-pulling or resetting: those are someone's unpushed contributions or a hand-modified engine, and no learner data is at risk in the meantime, so there is nothing to trade against.

**Alternatives:** npm distribution like GSD (rejected for now — primer is markdown plus stdlib Python, so npm buys dist-tags at the cost of a build/publish pipeline and a Node dependency the project otherwise doesn't have; revisit if non-git installs become common); a tarball updater for zip-download installs (deferred — the documented install is `git clone`, and a tarball path that must not clobber local edits is more machinery than the case currently justifies, so it's a clear refusal message instead); a `SessionStart` hook (rejected — see above); auto-updating without asking (rejected — it would mean pulling code onto someone's machine unprompted).

**Tradeoff:** the core must stay a git checkout to self-update, and the version marker is a file someone has to bump, in exchange for an update path that needs no git knowledge and cannot leave an instance half-migrated. **Where primer runs** was resolved along the way and documented in the README: skills load from `~/.claude/skills/` in the terminal CLI, the desktop app, and IDE extensions — so the desktop app is the better home for the visual layer, since the view page is a local file a real browser renders. Cloud and Cowork sessions load skills from the claude.ai account instead and can reach neither the private instance nor a `file://` page, so primer is a local-session tool by design.
