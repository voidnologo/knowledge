...
**Decision:** The rich figure channel is `$DATA_DIR/lessons/<domain>/<slug>.view.html` — a **single local file with zero external requests** (inline CSS/JS/SVG, `data:` URIs only), opened by the learner clicking a `file://` link the Primer prints. It is a **derived build product**: regenerated from the artifact, gitignored in the instance. And it is **validated before the learner sees it** — `render` runs five checks and exits non-zero with the specific problem.
