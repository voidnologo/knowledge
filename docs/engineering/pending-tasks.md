...
- [ ] **F-17 — `render` is per-file, `ascii --id` is per-figure, so a page-only figure cannot be delivered at its beat without exposing later ones.** Rendering the sidecar to deliver one `curve` emitted all five figures, including the next beat's caption, which named the mechanism the learner was being asked to derive. Captions and `predict` lines are ungated **by design**, so the reveal gate is no defence. This makes the F-15 fix structurally unavailable for `curve`, `state` and explorables. Worked around by staging unreached specs outside the sidecar — fiddly, undocumented, and it breaks the `.STATE.md` cross-machine resume guarantee since the sidecar is no longer complete mid-lesson. Wants `render --id <fig>` / `--only <ids>` / `--upto <id>`.
- [ ] **F-18 — the ASCII `sequence` renderer is not a diagram.** Learner caught it unprompted: "trying to sketch a mermaid chart but it didn't render as a chart." It is a flat list of arrow rows behind a vestigial participant header that implies lanes the rows never use — a table advertising itself as a diagram, which `visuals.md` forbids. Options: real lifelines with columns; drop the header and own the list; or make `sequence` page-only like `curve` and `state` (cheapest, arguably most honest — sequence diagrams need more width than a terminal beat should take). Note this is the **second** unprompted figure-delivery correction from this learner in two lessons.
- [ ] **F-19 — a blank is only valid relative to what has already been said.** A pre-authored `sequence` had its Darkness Point message blanked; by the time the beat arrived that cost had been named in the previous beat, so the blank asked an answered question. Retargeted live. `diagramming.md` should say that a blank must be re-checked against the conversation before rendering — same class as F-08's stale sidecar, one level down: a frozen blank against a moving conversation.
- [ ] **F-16 — `recalibrate-check` is date-granular, so a same-day recalibrate hides everything logged after it.** After the minor recalibrate it reported `misses=0` with seven calibration entries appended since, all same-date. Any miss logged on the same calendar day as a recalibrate is dropped from the trigger — and a heavy day is exactly when the evidence-triggered tier should fire. Compounds with F-01. Fix: order within a date, or record a position marker at recalibrate rather than a date.
- [ ] **Document the 400-char caps.** `sweep-record --note` has the same undocumented limit as `sources-add --why`, and `--why` additionally forbids `|`. List both with the commands in `tools/README.md` and `research-protocol.md`.
