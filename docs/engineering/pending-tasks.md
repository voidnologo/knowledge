...
**From the graphics-heavy second lesson (Coriolis combat) — the figure subsystem under real load:**
- [x] **F-17 — FIXED. `render` gains `--upto ID` and `--only ID`.** Rendering the sidecar to deliver one `curve` emitted all five figures, including the next beat's caption, which named the mechanism the learner was being asked to derive. Captions and `predict` lines are ungated **by design**, so the reveal gate is no defence. This makes the F-15 fix structurally unavailable for `curve`, `state` and explorables. Worked around by staging unreached specs outside the sidecar — fiddly, undocumented, and it breaks the `.STATE.md` cross-machine resume guarantee since the sidecar is no longer complete mid-lesson. `--upto` stops at the beat reached, `--only` takes explicit ids (repeatable), an unknown id errors and lists what is present, and a bare `render` still emits everything. Verified live on the Coriolis artifact: rendering `--upto` beat 1 keeps beat 1's caption and drops beats 2 and 5. Specs can now all live in the sidecar from the start, which restores `.STATE.md` as a complete cross-machine checkpoint.
- [x] **F-18 — FIXED. The ASCII `sequence` renderer now draws real lifelines.** Learner caught it unprompted: "trying to sketch a mermaid chart but it didn't render as a chart." It is a flat list of arrow rows behind a vestigial participant header that implies lanes the rows never use — a table advertising itself as a diagram, which `visuals.md` forbids. Took the first option: each participant owns a column, each message is an arrow between its two columns, direction is in the arrowhead rather than in word order, a lost message stops at a `╳` short of its target, self-calls draw as a stub, and labels sit right-aligned in one column past the last lane (a 34-char budget will not fit between two columns, and truncating would reintroduce F-14 somewhere new). Note this was the **second** unprompted figure-delivery correction from this learner in two lessons.
- [x] **F-19 — DOCUMENTED. A blank is only valid relative to what has already been said.** A pre-authored `sequence` had its Darkness Point message blanked; by the time the beat arrived that cost had been named in the previous beat, so the blank asked an answered question. Retargeted live. Now stated in both `diagramming.md` and the `lesson-protocol.md` figure beat: re-check the blank against the conversation immediately before rendering, and retarget or drop it if an intervening beat gave the answer away. Same class as F-08's stale sidecar, one level down.
- [x] **F-16 — FIXED, together with F-01 and F-02 (D-0030).** After the minor recalibrate it reported `misses=0` with seven calibration entries appended since, all same-date. Any miss logged on the same calendar day as a recalibrate is dropped from the trigger — and a heavy day is exactly when the evidence-triggered tier should fire. Counting now starts from a positional marker written by the new `recalibrate-mark` command into the calibration log itself, so there is no cross-file date comparison and no same-day ambiguity; instances without a marker fall back to a date comparison that is now `>=` rather than `>`, which recovers the same-day entries. The live instance went from reporting `misses=0` to `misses=8`.
- [x] **Document the 400-char caps — DONE.** `sweep-record --note` has the same undocumented limit as `sources-add --why`, and `--why` additionally forbids `|`. Both now listed in `research-protocol.md` alongside the `|` restriction.

**Confirmed working under real load (no action):** inline `ascii --id` at the beat for all four ASCII forms; the page-only fallback message for `curve` and `state` (clean, exit 0, caption preserved); the F-06 blanking fix on a live three-actor timeline with two blanked spans and a declared axis; the F-14 gutter fix (`requests 81-100` in full); the explorable formula compiler on `100 * (1 - pow(5 / 6, 2 * dice))` — correct emitted JS, gated invariant; all six `render` checks on every render; and the full ledger cycle on a cold domain (no recorded sweep → full sweep → 18 verdicts including 8 drops → `sweep-record` → floor accreted by promotion).
...
- [x] **F-01 — FIXED (D-0030).** Fired on "5 misses ≥ 4" when 2 of the 5 rows are annotated `(not a miss)`; real count 3. Count only rows whose miss-type is in the documented token vocabulary; treat the rest as annotations. Note the inversion this creates: a *mastery signal* currently increases the apparent need for correction.
- [x] **F-02 — DONE, plus a second token.** `feedback-protocol.md` instructs the Primer to infer "style confirmation" and there is no token to record it. Added `register-miss` and, from the second lesson, `analogy-transfer` — an analogy that carried the structure across correctly and the magnitude incorrectly, which is not `dead-analogy` (that one never connected) and needs a different correction: keep the analogy, fix the number. Both documented in the template and in `feedback-protocol.md`.
- [x] **F-13 — NOT A BUG. Retracted.** A `set -e` loop silently drops entries; the sweep agent caught three only by reading stdout. Rejects are the entries most worth keeping. Exit non-zero. (Error messages themselves are good — specific and actionable.)
...
- [x] **F-08 — FIXED. The `resume` verb now specifies the three re-checks.** The resumed sidecar was stale three independent ways: a cited source's domain no longer resolves; its primary probe telegraphed the answer in exactly the way a *later* calibration-log entry forbids; and its premise had been resolved by a lesson that ran after it was drafted. Resume should re-check `freshness_check` against the horizon, diff the sidecar's assumptions against calibration-log entries newer than its date, and confirm the premise against the current marker. Three reads, all three would have fired.
...
- [x] **F-12 — FIXED. Added `hatch-log`, `hatch-trend` and `recalibrate-mark` to SKILL.md's command list.** They exist only in `feedback-protocol.md`, which M1 made an on-demand read, so a run that never opens it can't know the hatch counter exists. Same risk applies to any command documented only in a deferred file — audit the set.
- [x] **F-10 follow-up — FIXED. SKILL.md step 3 now says to keep the plan non-spoiling.** **Plan** (step 3) precedes **Run the protocol** (step 4, containing Elicit), so a literal plan paragraph hands over the invariant the cold recall is meant to test. Say explicitly that the plan stays non-spoiling until after Diagnose.
